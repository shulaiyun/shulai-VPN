...
windows-zip-release:
	...
	mkdir -p SlothVPN; \
	unzip -q "$$ZIP_FILE" -d SlothVPN/; \
	rm "$$ZIP_FILE"; \
	tar -a -cf "$$FILE_NAME.zip" SlothVPN; \
	rm -rf SlothVPN; \
...
linux-appimage-release:
	...
	$(BLUE)Adding StartupWMClass to slothvpn.desktop$(DONE); \
	sed -i '/^\[Desktop Entry\]/a StartupWMClass=top.shulaiyun.slothvpn' "squashfs-root/slothvpn.desktop"; \
	...
	ARCH=x86_64 appimagetool --no-appstream squashfs-root SlothVPN.AppImage > /dev/null; \
	$(BLUE)Cleaning up squashfs$(DONE); \
	rm -rf squashfs-root; \
	$(YELLOW)Creating Portable Package$(DONE); \
	PKG_DIR_NAME="slothvpn-linux-appimage"; \
	$(BLUE)Creating dir: $$PKG_DIR_NAME$(DONE); \
	mkdir -p "$$PKG_DIR_NAME"; \
	$(BLUE)Moving SlothVPN.AppImage$(DONE); \
	cp -p "SlothVPN.AppImage" "$$PKG_DIR_NAME/SlothVPN.AppImage"; \
	$(BLUE)Creating Portable Home directory$(DONE); \
	mkdir -p "$$PKG_DIR_NAME/SlothVPN.AppImage.home"; \
