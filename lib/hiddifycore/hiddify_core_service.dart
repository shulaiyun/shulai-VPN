...
class HiddifyCoreService with InfraLogger {
  ...
  final Ref ref;
  static const List<Duration> _bgStartRetryDelays = [
  ...
  TaskEither<ConnectionFailure, Unit> start(String path, String name, bool disableMemoryLimit) {
    return TaskEither(() async {
      ...
      final startResult = await _startBackgroundCoreWithRetry(
        path: path,
        name: name,
        disableMemoryLimit: disableMemoryLimit,
      );
      if (startResult case Left(value: final failure)) {
  ...
  Future<Either<ConnectionFailure, Unit>> _startBackgroundCoreWithRetry({
    ...
    final maxAttempts = _bgStartRetryDelays.length + 1;

    for (var attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        final response = await core.bgClient.start(
          request,
          options: CallOptions(timeout: Duration(seconds: min(6 + attempt, 12))),
        );
        ref.read(coreRestartSignalProvider.notifier).restart();
        if (response.messageType != MessageType.ALREADY_STARTED && response.messageType != MessageType.EMPTY) {
          final alert = response.message.contains("denied") ? CoreAlert.requestVPNPermission : CoreAlert.startFailed;
          currentState = CoreStatus.stopped(
            alert: alert,
            message: "failed to start core ${response.messageType} ${response.message}",
          );
          statusController.add(currentState);
          return left(
            currentState.getCoreAlert() ??
                ConnectionFailure.unexpected(
                  "failed to start core ${response.messageType} ${response.message}",
                ),
          );
        }
        loggy.info("background core started on attempt ${attempt + 1}/$maxAttempts");
        return right(unit);
      } on GrpcError catch (error, stackTrace) {
        lastGrpcError = error;
        lastError = error;
        lastStackTrace = stackTrace;
        final retryable = _isRetryableBgStartError(error);
        final hasMoreAttempts = attempt < _bgStartRetryDelays.length;
        loggy.warning(
          "background core start grpc failure on attempt ${attempt + 1}/$maxAttempts "
          "code=${error.code} retryable=$retryable message=${error.message}",
        );
        if (!retryable || !hasMoreAttempts) {
          break;
        }
        await Future.delayed(_bgStartRetryDelays[attempt]);
      } catch (error, stackTrace) {
        lastError = error;
        lastStackTrace = stackTrace;
        loggy.error("failed to start bg core", error, stackTrace);
        break;
      }
    }
    ...
    if (lastGrpcError != null) {
      loggy.error("background core unavailable after retries: $lastGrpcError");
      if (lastGrpcError!.code == StatusCode.unavailable) {
        return left(const ConnectionFailure.unexpected("background core is not started yet!"));
      }
      return left(
        ConnectionFailure.unexpected(
          "failed to start background core (${lastGrpcError!.codeName}) ${lastGrpcError!.message ?? ''}".trim(),
        ),
      );
    }
    return left(ConnectionFailure.unexpected(lastError, lastStackTrace));
  }

  bool _isRetryableBgStartError(GrpcError error) {
  ...
  Stream<OutboundGroup?> watchGroup() async* {
    ...
    try {
      yield* core.bgClient.outboundsInfo(Empty()).map((event) => _pickOverviewGroup(event.items));
    } catch (e) {
  ...
  OutboundGroup? _pickOverviewGroup(List<OutboundGroup> groups) {
    if (groups.isEmpty) return null;
    if (groups.length == 1) return groups.first;
    OutboundGroup? best;
    var bestScore = -1 << 30;
    for (final group in groups) {
      final score = _scoreGroup(group);
      if (score > bestScore) {
        bestScore = score;
        best = group;
      }
    }
    return best ?? groups.first;
  }

  int _scoreGroup(OutboundGroup group) {
    final tag = group.tag.toLowerCase();
    final type = group.type.toLowerCase();
    final leafCount = group.items.where((item) => !item.isGroup).length;
    final selectedBonus = group.selected.isNotEmpty ? 20 : 0;
    var score = leafCount * 100 + group.items.length * 10 + selectedBonus;
    if (tag.contains('select') || tag.contains('proxy') || tag.contains('auto') || tag.contains('best')) {
      score += 500;
    }
    if (type.contains('selector') || type.contains('urltest') || type.contains('loadbalance')) {
      score += 350;
    }
    return score;
  }
