...
class GeneralPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final useXrayTitle = locale == AppLocale.zhCn
        ? '尽可能使用 xray-core'
        : t.pages.settings.general.useXrayCoreWhenPossible;
    final useXrayMsg = locale == AppLocale.zhCn
        ? '解析订阅链接时使用 xray-core。您需要重新导入订阅链接才能启用此选项'
        : t.pages.settings.general.useXrayCoreWhenPossibleMsg;
    ...
    return Scaffold(
      ...
      body: SlothSettingsList(
        children: [
          ...
          SwitchListTile.adaptive(
            title: Text(useXrayTitle),
            subtitle: Text(useXrayMsg),
