...
class ProfileTile extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Card(
      margin: margin,
      color: color ?? (profile.active ? theme.colorScheme.surfaceContainer : theme.colorScheme.surfaceContainerLow),
                      ...
                      child: Column(
                        ...
                        children: [
                          if (isMain)
                            Row(
                              children: [
                                const SlothIcon(SlothIconType.subscription, size: 16),
                                const Gap(6),
                                Expanded(
                                  child: Text(
                                    profile.name,
                            ...
                            Row(
                              children: [
                                if (profile.active) ...[
                                  const SlothIcon(SlothIconType.security, size: 14),
                                  const Gap(6),
                                ],
                                Expanded(
                                  child: Text(
                                ...
                                if (profile.active) ...[
                                  const Gap(8),
                                  const DecoratedBox(
                                    decoration: BoxDecoration(shape: BoxShape.circle, color: SlothPalette.success),
...
class RemainingTrafficIndicator extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final color = ratio < 0.45
        ? SlothPalette.success
        : ratio < 0.75
        ? SlothPalette.warning
        : SlothPalette.danger;
